...
La app es un **ERP/CRM interno para un negocio de educación online** ("Growth Ops" / [tenant]-os), con dos sistemas coexistiendo:
...
### Sistema legacy: "Lanzamiento / Cold Calling / [tenant]" (~1.450 líneas)
...
6. Dos despliegues de marca ([tenant] y [tenant]) mantenidos copiando archivos y re-aplicando migraciones a mano por feature — sin límite de paquete/módulo compartido, riesgo estructural de divergencia.
...
### P2
- Secretos con fallback hardcodeado: `DASHBOARD_PASSWORD` cae a `'[tenant]2026'` si la env var no está seteada; `CC_SESSION_SECRET` cae a `'cc-secret-fallback-2026'` — ambos fallan "abierto" en vez de fallar cerrado.
