...
| --- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| 1   | **P0** | Regresión de F-1: `20260920120000` recreó la RPC del RAG de 6 argumentos con la puerta vieja (rol sin subcuenta). Un admin de un cliente podía leer el conocimiento de otro. Además nació ejecutable por `anon`. | Migración `20260921120000` + `tests/rag-gate-subcuenta.test.mjs` (mira el estado FINAL). **Falta aplicarla.** |
...
**Los 12 siguen sin registrar.** Es trabajo humano: el aviso dice dónde ir en cada caso (cuota →
"Registrar cobro" en la venta con la referencia `pi_…`; venta nueva → Integraciones › Stripe).

## 3. Abierto, con dueño

| Prio   | Hallazgo                                                                                                 | Dueño / destino                                                |
| ------ | -------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| **P0** | Aplicar `20260921120000` en producción                                                                   | **Confirmación de Alex**, después la aplico yo                 |
| P1     | Registrar los 12 pagos y la devolución de 50 €                                                           | **Alex / finanzas**                                            |
