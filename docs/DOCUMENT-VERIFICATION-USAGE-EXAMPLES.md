...
To finalize your enrollment in the [tenant] program, please provide a photo of your identification document.

📸 Upload Your Document: https://tu-dominio.com/documentos/verify/abc123xyz789
...
curl -X POST https://tu-dominio.com/api/evergreen/contracts/student \
...
```bash
# 1. Upload document
curl -X POST https://tu-dominio.com/api/evergreen/documents/verify \
  -H "Content-Type: application/json" \
  -d '{
    "saleId": "test-sale-id",
...
curl -X POST https://tu-dominio.com/api/evergreen/documents/override \
  -H "Content-Type: application/json" \
  -d '{
    "saleId": "test-sale-id",
