...
Orden de ejecución: S0 tramo 1 → A0 → F-1 → **F6** → S0 tramo 2 → F1 → F2 → F3 → F7 → F4 → IA cualitativa (llamadas, transcripciones, mensajes, RAG) → F5 → F0 → F8 → F9. (F6 adelantado — ver Enmienda 1 al final de esta sección.) Una sola fase activa a la vez. Los meses son orientativos; manda el orden y la graduación.
...
| F6 (adelantada)          | Privacidad y `erase_person`; debe cerrarse antes de ingerir transcripciones, mensajes o llamadas al RAG                                        | Sin PII recuperable en stores activos; informe de borrado verificable                                                                          |
...
### Enmiendas (2026-09-20)

Acordadas entre Alex, el agente que venía ejecutando en `origin/main` y esta sesión, tras la
auditoría del estado real. Se registran aquí porque la constitución debe describir lo que de
verdad pasa, no una intención que nadie sigue.

**Enmienda 1 — F6 se adelanta a justo después de F-1.** F6 no depende de ninguna fase previa y
hoy hay 972 personas reales con PII en claro en `contacts` y sin `erase_person`. Dejarlo en el
mes 5 era una decisión sin justificación técnica. El resto del orden no cambia.

**Enmienda 2 — regla interina sobre el RAG, vigente desde hoy.** Quedan **congelados los
pipelines de ingesta con PII** al RAG (transcripciones de llamadas, DMs, mensajes de personas
reales) hasta que F6 esté cerrada. El corpus de **conocimiento de plataforma sin PII** (las
skills de `.agents/skills/*/SKILL.md`, guiones, fórmulas y frameworks) **sí puede seguir
creciendo**: es texto autoral, no datos de clientes.

Contexto de por qué hace falta la distinción: los PR #81 a #95 construyeron el RAG híbrido antes
de F6, lo que a primera vista incumple la línea roja. Verificado que no: `knowledge_chunks` solo
contiene skills de plataforma (`source` lo documenta y la ruta `ai/knowledge` es de solo
lectura). La línea roja no se ha cruzado, y esta enmienda la deja escrita para que no se cruce
por accidente.

**Enmienda 3 — reconocimiento del desvío.** Entre el 13 y el 20 de septiembre el trabajo no
siguió el roadmap por fases: fueron encargos directos de Alex (colaboradores, base neta de
comisión, RAG, capa orgánica). Se anota como hecho, no como incumplimiento. A partir de aquí,
el trabajo que no siga el orden de fases debe justificarse como P0/P1 o entrar por el Feature
Admission Gate de §9.
