...
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | ----------------------------------------------------------------------------------------------- | ------ |
| Freebuff (Buffy)  | **Test de regresión del escenario Apify con fallo/reintento**: ejercita `processRunResults` real con Supabase falso (CAS genuino) y `fetch` interceptado — sin red ni credenciales. Pendiente de producto no bloqueante de la unidad audit-integrity (PR #233) | test/apify-retry-scenario | `tests/apify-retry-scenario.test.mjs` (nuevo), `package.json` (solo flag `--experimental-transform-types` en el script `test`), `docs/ACTIVE_HANDOFF.md` | 26-sep |
