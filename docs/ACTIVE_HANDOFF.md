...
2. **Ampliado el diagnóstico de drift real**: `list_migrations` de Supabase solo registra 7 migraciones aplicadas de las 20 que hay en `supabase/migrations/`. Las 14 restantes están en dos categorías distintas — no asumir que "no está en el historial" = "no está en el esquema":
   ...
   - **`drop_partners` — VERIFICADA pero NO aplicada, requiere tu confirmación explícita**: `partners` tiene exactamente 3 filas ([tenant] 55%, Alex 30%, [tenant] 15%, todas `is_active=true`, sin `user_id` vinculado), confirmado que no hay ninguna FK de otra tabla apuntando a `partners`. Coincide con lo que la propia migración documenta (`SAFE_TO_REMOVE`). Es un `DROP TABLE` — destructivo e irreversible una vez aplicado — así que no lo ejecuté sin tu autorización explícita aunque la verificación salió limpia. Si confirmas, es un `apply_migration` de una sola línea (`DROP TABLE IF EXISTS public.partners;`).
