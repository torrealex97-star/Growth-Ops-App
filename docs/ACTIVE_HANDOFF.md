...
| Freebuff (Buffy)  | **Data Health — control de silencio para Calendly y Stripe**: generaliza `saludWebhook` (24h sin recepción) a los tres webhooks entrantes, reutilizando la evidencia canónica de Integraciones | `feat/data-health-silencio-calendly-stripe` | `lib/data-health/webhooks.ts`, `app/api/[tenant]/evergreen/settings/data-health/route.ts`, `components/settings/DataHealthPanel.tsx`, `tests/data-health-sobres-webhooks.test.mjs` | 24-sep |
