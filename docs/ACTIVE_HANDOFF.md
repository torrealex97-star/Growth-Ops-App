...
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | ----------------------------------------------------------------------------------------------- | ------ |
| Freebuff (Buffy)  | Correcciones de integridad y permisos: venta-contacto en documentos, tenant scope de Drops, escritura de ajustes de correo para liderazgo y errores/reintentos idempotentes de Apify | fix/audit-integrity-guards | `documents/verify`, `drops/page.tsx`, APIs `settings/email*`, `lib/social/apify.ts`, webhook Apify, tests focalizados, este relevo | 26-sep |
...
**Estado: quality gate completo PASS en local (26-sep); pendiente commit, PR, CI, merge y relevo del tablero.** No se ejecutaron migraciones ni se consultó/escribió producción.
...
**Siguiente paso exacto (relevo):** 1) `git fetch origin --prune` y confirmar que `origin/main` sigue en `b87ab41` o avanzar la rama si movió; 2) commitear los 13 ficheros con pathspec acotado (NUNCA `git add -A`) desde `.worktrees/audit-integrity`; 3) push de `fix/audit-integrity-guards` (abortar si lo rechaza; comparar antes); 4) abrir PR a `main` y esperar CI completo verde diagnosticando con `gh run list --commit <sha>` (un run cancelled no es fallo: validar el del último SHA); 5) merge, borrar rama remota y local, fast-forward del `main` local; 6) re-verificar `npm run quality` en `main` fusionada (si el sandbox arranca sin swap, repetir la receta de swap de arriba; nunca debilitar cobertura); 7) borrar la fila de esta rama en el tablero (sección Trabajo en paralelo) y anotar el resultado en el estado. Pendiente de producto no bloqueante: prueba del escenario Apify con fallo/reintento (el fix de `items` no definido está incluido: `const items = batch as FilaJson[]` en `fetchDatasetItems`). No se ha creado commit, PR, push ni merge todavía; no se tocó el WIP del checkout principal ni el trabajo ajeno.
