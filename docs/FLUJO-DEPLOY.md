...
|----------------|---------------------------------------|------------------------------|--------------------------------|
| **Producción** | `iawinners.com`                       | Producción (datos reales)    | Clientes / equipo real         |
| **Staging**    | `iawinners-staging.vercel.app`        | Staging `xryk…` (aislada)    | El equipo prueba aquí (QA)     |
...
- **URL:** https://iawinners-staging.vercel.app
- **Login de la app (usuario):** `equipo@iawinners.com` / `Staging2026!`  (rol admin)
- **Password del panel simple** (secciones tipo `/login`, coldcalling): `staging2026`

## Cómo circula un cambio

```
1. Se hace el cambio de código (Adrián / desarrollo)   -> rama "staging" en git
2. Se despliega a STAGING:  vercel deploy               -> iawinners-staging.vercel.app
3. El equipo prueba en staging y confirma que va bien
4. Adrián da el OK
5. Se despliega a PRODUCCIÓN el MISMO código:  vercel --prod  -> iawinners.com
...
vercel alias set <url-del-preview> iawinners-staging.vercel.app   # URL fija

# 2) Cuando el equipo confirma OK -> subir a producción (mismo código)
git checkout main && git merge staging && git push origin main
vercel --prod                                   # -> iawinners.com
#   y anotar el cambio en CHANGELOG.md
```

## Estado de la infraestructura (2026-07-13)

- Repo privado GitHub: `academiaiawinners-netizen/iawinners-app` (ramas `main` y `staging`).
- Vercel: proyecto `iawinners-app`. Deploys por CLI (la conexión automática Git↔Vercel
