...
> **ESTADO: IMPLEMENTADO y desplegado a producción el 2026-07-08** (iawinners-app.vercel.app).
> Migración v29 aplicada. Falta solo configurar 2 env vars en Vercel para activar email y
> webhook de accesos (ver §5.1 y §5.3): `RESEND_API_KEY` + `RESEND_FROM=app@iawinners.com` y
...
> (Adri / Jesús / Alex) y en el estado real del código de `iawinners-os`.
>
> **Regla de oro acordada en la llamada:** simpleza y centralización. Todo el proceso
> (contrato, correo, cortafuegos, tracking) vive dentro de la app IA WINNERS. GoHighLevel
...
│  2. Closer → App IA WINNERS → "Nueva Venta"                                │
...
│  4. Contrato se ENVÍA por email (Resend, app@iawinners.com)                │
...
├─────────────────────────────────────────────────────────────────────────┤
│  6. App IA WINNERS → WEBHOOK SALIENTE → GoHighLevel  ← NUEVO               │
...
1. **Plantilla "Contrato de alumno"** (nueva plantilla en `contract_templates`, reutilizando
   ...
   - Mini-plantilla: logo IA WINNERS + "Bienvenido a nuestra academia. Acepta las condiciones
...
- Configurar `RESEND_FROM = app@iawinners.com` (dominio nuevo, sin lanzamientos → buena
...
### 5.4 Simplificación del acceso (idea de Jesús)
...
| 6 | Envío del contrato por Resend desde `app@iawinners.com` | Config + código | `lib/email/resend.ts`, env `RESEND_FROM` |
...
### 8.2 12 meses / 3.000 €
- Pendiente de confirmar por Jesús si se crea ya (oculto) o se omite. No bloquea el arranque
