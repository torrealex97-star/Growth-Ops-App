# MEJORAS DESDE HOY (2026-07-02) — IA WINNERS
> Changelog de cambios/mejoras a partir del 2026-07-02, para luego **portar a The Closer Club**.
...
- Portar a Closer Club: mismos ficheros + migración v19; credenciales y cuenta propias de Closer Club.
...
- ✅ CONECTADO (2026-07-02): suscripción webhook **org-scope** creada en Calendly de IA WINNERS (org `50d51e10-fdca-42ca-8f72-ad3fad99391b`), eventos invitee.created/canceled, state active. Signing key en `CALENDLY_WEBHOOK_SECRET` (Vercel prod + local) → firma verificada. Captura TODAS las agendas de todos los setters. ⚠️ El PAT de Calendly se compartió en chat → rotar.
...
- **Para portar a Closer Club:** aplicar migración v12 a qgjr (vía Management API, sin seeds de datos) + copiar los archivos citados + rebrand + deploy. (Mismo procedimiento que el port inicial.)
...
- **Portar a Closer Club**: copiar `lib/filters/period.ts`, `components/os/PeriodFilterBar.tsx` + las páginas citadas.

### ⬜ PARA PORTAR A CLOSER CLUB — batch Calendly + Enlaces (secciones 9 y 10)
...
3. **Config externa en Closer Club:** crear su propia suscripción de webhook org-scope en su Calendly + setear `CALENDLY_WEBHOOK_SECRET`, `CALENDLY_API_TOKEN` en su Vercel. Rebrand donde aplique. Deploy.

### ⬜ PARA PORTAR A CLOSER CLUB — batch Pagos (sección 11)
...
- **Sigue ⬜:** desplegar worker de transcripción a Railway; **rotar** tokens compartidos en chat (CALENDLY_API_TOKEN, personal access token); portar batches 9-10 a Closer Club (ver checklist de arriba).
...
**Portar a Closer Club:** copiar `app/evergreen/sales/new/page.tsx`. El backfill de datos es específico de la BBDD de IA WINNERS.
...
**Portar a Closer Club:** `lib/commissions/{calculator,generate}.ts`, `app/api/evergreen/commissions/future/route.ts`, `app/api/evergreen/refunds/create/route.ts`, `app/api/evergreen/cron/reminders/route.ts`, `app/evergreen/commissions/page.tsx`, `app/evergreen/dashboard/page.tsx`.
...
**Portar a Closer Club:** copiar esos archivos. Tras portar, ejecutar una vez `POST /api/evergreen/sales/reconcile-all` con su `CRON_SECRET` para cuadrar. Sin migración nueva.
...
**Portar a Closer Club:** copiar los 2 archivos (mismo doble-FK en su esquema).
...
**Portar a Closer Club:** copiar `pnl/page.tsx` y `finanzas/page.tsx`.
...
- **Portar a Closer Club:** copiar `sales/page.tsx`.
