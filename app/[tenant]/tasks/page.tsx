...
const PRIORITIES: { value: Priority; label: string; badge: string; select: string }[] = [
  { value: 'baja', label: 'Baja', badge: 'bg-zinc-500/20 text-foreground border-border/30', select: 'border-border' },
  {
    ...
    select: 'border-blue-700/50',
  },
  {
    value: 'alta',
    label: 'Alta',
    badge: 'bg-amber-500/20 text-amber-400 border-amber-500/30',
    select: 'border-amber-700/50',
  },
  {
    value: 'urgente',
    label: 'Urgente',
    badge: 'bg-red-500/20 text-red-400 border-red-500/30',
    select: 'border-red-700/50',
  },
]
...
export default function TasksPage() {
  ...
  const createTask = async () => {
    ...
    const res = await fetch(`/api/${tenant}/evergreen/tasks`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        title: nt.title.trim(),
        description: nt.description || null,
        assignee_id: nt.assignee_id || null,
        stage: nt.stage,
    ...
    if (!res.ok) {
      toast.error('Error al crear', { description: d?.error })
      return
    }
  ...
  const generateFromTranscript = async () => {
    if (transcript.trim().length < 20) {
      toast.error('Pega una transcripción más larga')
      return
    }
    setAiLoading(true)
    const res = await fetch(`/api/${tenant}/evergreen/tasks/from-transcript`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    setAiLoading(false)
    if (!res.ok) {
      toast.error('No se pudieron generar tareas', { description: d?.error })
      return
    }
    setAiSummary(d.summary || '')
    setProposals(
      (d.proposals || []).map(
        (p: { title: string; description: string; assignee_id: string | null; stage: string | null }) => ({
          selected: true,
          title: p.title,
  ...
  const confirmProposals = async () => {
    ...
    setAiCreating(false)
    if (!res.ok) {
      toast.error('Error al crear tareas', { description: d?.error })
      return
    }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold text-foreground flex items-center gap-2">
            <ListChecks className="w-6 h-6 text-white" /> Tareas
          </h1>
        ...
        <div className="flex items-center gap-3">
          ...
          {isAdmin && (
            <div className="flex rounded-lg border border-border overflow-hidden text-sm">
              <button
                ...
                className={`px-3 py-2 ${scope === 'todas' ? 'bg-white text-black' : 'bg-card text-muted-foreground hover:text-foreground'}`}
              >
                Todas
              </button>
              <button
                onClick={() => setScope('mias')}
                className={`px-3 py-2 ${scope === 'mias' ? 'bg-white text-black' : 'bg-card text-muted-foreground hover:text-foreground'}`}
              >
                Mías
              </button>
            </div>
          ...
          {isAdmin && (
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm bg-cyan-600/20 text-cyan-300 border border-cyan-600/40 hover:bg-cyan-600/30 whitespace-nowrap"
            >
          ...
          {isAdmin && (
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 rounded-[10px] text-sm bg-white text-black hover:bg-zinc-200 whitespace-nowrap"
            >
            ...
            return (
              ...
              >
                ...
                <div className="space-y-2 min-h-[40px]">
                  ...
                  {cards.length === 0 && (
                    <p className="text-xs text-muted-foreground text-center py-4">Suelta aquí una tarea</p>
                  )}
                </div>
              </div>
            )
        ...
        >
          ...
          >
            ...
            <Field label="Asignar a">
              <select
                value={nt.assignee_id}
                onChange={(e) => setNt({ ...nt, assignee_id: e.target.value })}
                className={inputCls}
              >
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Etapa">
                <select
                  value={nt.stage}
                  onChange={(e) => setNt({ ...nt, stage: e.target.value as Stage })}
                  className={inputCls}
                >
                  {STAGES.map((s) => (
              ...
              <Field label="Prioridad">
                <select
                  value={nt.priority}
                  onChange={(e) => setNt({ ...nt, priority: e.target.value as Priority })}
                  className={inputCls}
                >
                  {PRIORITIES.map((p) => (
                    <option key={p.value} value={p.value}>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button onClick={createTask} className="px-3 py-2 text-sm bg-brand-600 text-white rounded-lg">
                Crear
              </button>
            </div>
          </div>
            ...
            {proposals.length === 0 ? (
              <>
                ...
                <div className="flex justify-end gap-2">
                  <button onClick={() => setShowAI(false)} className="px-3 py-2 text-sm text-muted-foreground">
                    Cancelar
                  </button>
                  <button
                    onClick={generateFromTranscript}
                    disabled={aiLoading}
                    className="flex items-center gap-2 px-3 py-2 text-sm bg-cyan-600 text-foreground rounded-lg disabled:opacity-50"
                  >
                    {aiLoading ? (
                      <>
                      ...
                      <>
                        <Sparkles className="w-4 h-4" /> Generar propuestas
                      </>
                    )}
                  </button>
                </div>
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground">
                  Revisa, edita y selecciona las tareas antes de crearlas. Solo se crearán las marcadas.
                </p>
                <div className="space-y-2">
                  {proposals.map((p, i) => (
                    <div
                      key={i}
                      className={`rounded-lg border p-3 space-y-2 ${p.selected ? 'border-cyan-600/40 bg-cyan-600/5' : 'border-border bg-card/50 opacity-60'}`}
                    >
                      <div className="flex items-start gap-2">
                        <input
                          ...
                          className="mt-1 h-4 w-4 rounded border-border bg-card accent-cyan-500"
                        />
                        <input
                          value={p.title}
                          onChange={(e) =>
                      ...
                      <textarea
                        ...
                        rows={2}
                        className="w-full bg-muted border border-border rounded p-2 text-xs text-foreground"
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <select
                          ...
                          onChange={(e) =>
                            setProposals((prev) =>
                              prev.map((x, j) => (j === i ? { ...x, assignee_id: e.target.value } : x))
                            )
                          }
                          className="text-xs rounded border border-border bg-muted text-foreground px-2 py-1.5"
                        >
                          <option value="">— sin asignar —</option>
                          {users.map((u) => (
                            <option key={u.id} value={u.id}>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setProposals((prev) =>
                              prev.map((x, j) => (j === i ? { ...x, stage: e.target.value as Stage } : x))
                            )
                          }
                          className="text-xs rounded border border-border bg-muted text-foreground px-2 py-1.5"
                        >
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setProposals((prev) =>
                              prev.map((x, j) => (j === i ? { ...x, priority: e.target.value as Priority } : x))
                            )
                          }
                          className="text-xs rounded border border-border bg-muted text-foreground px-2 py-1.5"
                        >
                ...
                <div className="flex justify-between items-center gap-2 pt-1">
                  ...
                  <button
                    ...
                    disabled={aiCreating}
                    className="flex items-center gap-2 px-3 py-2 text-sm bg-brand-600 text-white rounded-lg disabled:opacity-50"
                  >
                    {aiCreating ? (
                      <>
                      ...
                      <>Crear {proposals.filter((p) => p.selected).length} tarea(s)</>
                    )}
                  </button>
                </div>
              </>
