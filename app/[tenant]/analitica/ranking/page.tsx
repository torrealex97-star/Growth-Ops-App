  ...
  for (const a of appointments) {
    ...
    const row =
      ...
      map
        ...
        .get(ownerId)!;
    row.total += 1;
    if (a.offered === true || isAttended(a.status)) {
      row.qualified += 1;
    }
...
export default function PipelinePage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [
        ...
        supabase
          .from("contacts")
          .select("id, created_at, first_contact_at, lead_status"),
        supabase
          .from("appointments")
          .select(
        ...
        supabase
          .from("sales")
          .select(
            "id, gross_amount, status, sale_date, closer_id, setter_id, contact_id",
          ),
        supabase
          .from("collections")
      ...
      setUsersWithRoles(
        (usersRolesRes.data as unknown as UserWithRoleRow[]) || [],
      );
  ...
  const filteredAppointments = useMemo(() => {
    return appointments.filter(
      (a) =>
        (personId === "all" ||
          a.setter_id === personId ||
          a.closer_id === personId) &&
  ...
  const filteredSales = useMemo(() => {
    return sales.filter(
      (s) =>
        (personId === "all" ||
          s.closer_id === personId ||
          s.setter_id === personId) &&
  ...
  const salesForPerson = useMemo(
    () =>
      personId === "all"
        ? sales
        : sales.filter(
            (s) => s.closer_id === personId || s.setter_id === personId,
          ),
    [sales, personId],
  );
  ...
  const filteredCollections = useMemo(() => {
    ...
    return collections.filter(
      (c) =>
        (personId === "all" || saleIds.has(c.sale_id)) &&
    ...
    const byPerson =
      personId === "all"
        ...
        : (() => {
            ...
            for (const a of appointments) {
              if (
                (a.setter_id === personId || a.closer_id === personId) &&
                a.contact_id
              )
                contactIds.add(a.contact_id);
            }
            for (const s of sales) {
              if (
                (s.closer_id === personId || s.setter_id === personId) &&
                s.contact_id
              )
                contactIds.add(s.contact_id);
            }
  ...
  const funnel = useMemo(() => {
    ...
    const cohortAppointments = appointments.filter(
      (a) =>
        a.contact_id &&
        cohortIds.has(a.contact_id) &&
        matchesPerson(a.setter_id, a.closer_id),
    );
    const cohortSales = sales.filter(
      (s) =>
        s.contact_id &&
        cohortIds.has(s.contact_id) &&
        matchesPerson(s.setter_id, s.closer_id),
    );
    ...
    const ofertas = new Set(
      cohortAppointments
        .filter((a) => resolverOferta(a).valor === true)
        .map((a) => a.contact_id),
    ).size;
    const cierres = new Set(
      cohortSales
        .filter((s) => s.status === "active" || s.status === "partial_refund")
        .map((s) => s.contact_id),
    ).size;
  ...
  const pctOf = (curr: number, prev: number) =>
    prev ? (curr / prev) * 100 : null;
  ...
  const closers = useMemo(
    () => teamRanking(filteredSales, [], usersWithRole, "closer"),
    [filteredSales, usersWithRole],
  );
  const setters = useMemo(
    () => teamRanking(filteredSales, [], usersWithRole, "setter"),
    [filteredSales, usersWithRole],
  );
  ...
  const targetProgress = useMemo(() => {
    ...
    return targets
      .map((t) => {
        const current = targetCurrentValue(
          t,
          { sales, collections, appointments },
          todayStr,
        );
  ...
  const hasData =
    contacts.length > 0 || appointments.length > 0 || sales.length > 0;
        ...
        <>
                  ...
                  const textColor =
                    pct >= 100
                      ...
                      : pct >= 60
                        ? "text-amber-400"
                        : "text-red-400";
                  const isCurrency = ["revenue", "cash_collected"].includes(
                    t.metric_key,
                  );
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Conversión global Lead → Cierre:{" "}
              {pctOf(funnel.cierres, funnel.leads)?.toFixed(1) ?? "—"}%
            </p>
            <p className="text-[11px] text-muted-foreground mt-1">
              Por cohorte: cada etapa cuenta a los mismos leads creados en el
              periodo elegido, mirando qué les pasó después (sin importar
              cuándo). Puede tardar en reflejar cierres de leads muy recientes.
            </p>
          </div>
