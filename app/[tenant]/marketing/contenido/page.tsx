...
const TYPES = [
  ...
  { value: 'video_largo', label: 'Vídeo largo' },
...
export default function ContentPage() {
  ...
  const sorted = useMemo(() => {
    ...
    return [...items].sort((a, b) => {
      ...
      if (sort === 'publish_asc') {
        const ad = a.publish_date || '9999-12-31',
          bd = b.publish_date || '9999-12-31'
    ...
    setItems((prev) =>
      prev.map((c) => {
        ...
        if (field === 'assigned_to')
          next.assignee = value ? { full_name: users.find((u) => u.id === value)?.full_name || '' } : null
        return next
      })
    )
  }
  ...
  const moveItem = async (id: string, direction: 'up' | 'down') => {
    ...
    if (r1.error || r2.error)
      toast.error('No se pudo reordenar', { description: r1.error?.message || r2.error?.message })
  }
  ...
  const create = async () => {
    ...
    const { error } = await supabase.from('content_items').insert({
      ...
      link_url: nc.link_url || null,
      publish_date: nc.publish_date || null,
    ...
    setShowNew(false)
    setNc({ title: '', content_type: 'reel', link_url: '', publish_date: '', assigned_to: '', notes: '' })
    load()
  }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <Clapperboard className="w-6 h-6 text-brand-400" /> Contenido
          </h1>
          <p className="text-muted-foreground text-sm mt-1">
            Pipeline editorial — idea → guionizado → grabado → editando → editado → publicado
          </p>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex rounded-lg border border-border overflow-hidden">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 text-sm ${view === 'tabla' ? 'bg-brand-600 text-white' : 'bg-card text-muted-foreground hover:text-foreground'}`}
            >
              <Table2 className="w-4 h-4" /> Tabla
            </button>
            <button
              onClick={() => setView('kanban')}
              className={`flex items-center gap-1.5 px-3 py-2 text-sm ${view === 'kanban' ? 'bg-brand-600 text-white' : 'bg-card text-muted-foreground hover:text-foreground'}`}
            >
        ...
        {view === 'tabla' && (
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 text-sm rounded-lg border border-border bg-card text-foreground px-3 py-2 hover:bg-card/70"
            >
              <SlidersHorizontal className="w-4 h-4" /> Columnas
              <span className="text-xs text-muted-foreground">
                ({COLUMNS.filter((c) => cols[c.key]).length}/{COLUMNS.length})
              </span>
            </button>
            {showCols && (
              <>
                ...
                <div className="absolute left-0 mt-2 z-40 w-56 bg-card border border-border rounded-lg p-2 shadow-xl">
                  ...
                  {COLUMNS.map((col) => (
                    <label
                      key={col.key}
                      className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-muted cursor-pointer text-sm text-foreground"
                    >
                      <input
                        type="checkbox"
      ...
      {loading ? (
        ...
        <>
          <TableView
            ...
            persist={persist}
            onOpen={setDetailId}
            onMove={sort === 'manual' ? moveItem : undefined}
          />
  ...
  return (
    <div className="rounded-xl border border-border bg-card/50 p-4">
      <p className="text-xs uppercase tracking-wider text-muted-foreground mb-2">
        Total por mes (precio de las piezas)
      </p>
...
function TableView({
  ...
  const testimonioCell = (c: Content) => (
    <td className="px-2 py-1 align-middle min-w-[170px]">
      ...
      >
        ...
        {testimonios.map((t) => (
          <option key={t.id} value={t.id} className="bg-muted text-foreground">
            {t.name}
          </option>
        ))}
      </select>
      ...
      <input
        ...
        onBlur={(e) => {
          if ((e.target.value || null) !== value) {
            patch(id, field, e.target.value || null)
            persist(id, field, e.target.value || null)
          }
        }}
  ...
  const readCell = (id: string, value: string | null, placeholder: string) => (
    <td className="px-2 py-1 align-middle min-w-[150px] max-w-[220px]">
      <button
        onClick={() => onOpen(id)}
        className="block text-left w-full text-sm text-foreground hover:text-foreground border border-transparent hover:border-border rounded px-2 py-1 truncate"
      >
        {value ? (
          <span className="truncate">{value.replace(/\s+/g, ' ')}</span>
        ) : (
  ...
  const editorCell = (c: Content) => (
    <td className="px-2 py-1 align-middle min-w-[150px]">
      <select
        ...
        onChange={(e) => {
          const v = e.target.value || null
          patch(c.id, 'assigned_to', v)
          persist(c.id, 'assigned_to', v)
        }}
      ...
      >
        ...
        {users.map((u) => (
          <option key={u.id} value={u.id} className="bg-muted text-foreground">
            {u.full_name}
          </option>
        ))}
      </select>
        ...
        <tbody>
          {items.map((c, i) => (
            <tr key={c.id} className="border-b border-border/60 hover:bg-card/30">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <div className="flex flex-col -space-y-1">
                      <button
                        ...
                        disabled={i === 0}
                        className="text-muted-foreground hover:text-brand-400 disabled:opacity-20"
                        title="Subir"
                      >
              ...
              <td className="px-2 py-1 align-middle min-w-[220px] max-w-[300px]">
                <div className="flex items-center gap-1.5">
                  ...
                  <input
                    ...
                    onBlur={(e) => {
                      if ((e.target.value || null) !== c.title) {
                        patch(c.id, 'title', e.target.value || null)
                        persist(c.id, 'title', e.target.value || null)
                      }
                    }}
              ...
              {cols.solution_explanation &&
                cell(c.id, 'solution_explanation', c.solution_explanation, 'Qué resuelve…', 'min-w-[200px]')}
                ...
                <td className="px-2 py-1 align-middle">
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      persist(c.id, 'status', e.target.value)
                    }}
                    className={`text-xs rounded px-2 py-1 ${STATUS_COLORS[c.status] || 'border border-border bg-muted text-foreground'}`}
                  >
                    {STATUSES.map((s) => (
                      <option key={s.value} value={s.value} className="bg-muted text-foreground">
                        {s.label}
                      </option>
                    ))}
                  </select>
              ...
              {cols.publish_date && (
                <td className="px-2 py-1 align-middle">
                  <input
                    type="date"
                    defaultValue={c.publish_date || ''}
                    onBlur={(e) => {
                      patch(c.id, 'publish_date', e.target.value || null)
                      persist(c.id, 'publish_date', e.target.value || null)
                    }}
                    className="text-xs rounded border border-border bg-muted text-foreground px-2 py-1"
                  />
                </td>
              )}
          ...
          {items.length === 0 && (
            <tr>
              <td colSpan={colCount} className="text-center text-muted-foreground py-10">
                Aún no hay contenido. Crea una pieza o añádela desde Competencia.
              </td>
            </tr>
          )}
        </tbody>
        ...
        return (
          <div key={col.value} className="bg-card/50 border border-border rounded-lg p-3">
            <div className="flex items-center justify-between mb-3">
              <span
                className={`text-xs font-semibold px-2 py-1 rounded ${STATUS_COLORS[col.value] || 'bg-muted/60 text-foreground'}`}
              >
                {col.label}
              </span>
            ...
            <div className="space-y-2">
              {cards.map((c) => (
                ...
                >
                  ...
                  {c.notes && (
                    <p className="text-xs text-muted-foreground line-clamp-2 whitespace-pre-wrap">{c.notes}</p>
                  )}
                  ...
                  {c.our_reel_url && (
                    ...
                    >
                      Nuestro reel <ExternalLink className="w-3 h-3" />
                    </a>
                  )}
                  ...
                  <select
                    ...
                    className={`w-full text-xs rounded px-2 py-1 ${STATUS_COLORS[c.status] || 'border border-border bg-muted text-foreground'}`}
                  >
                    {STATUSES.map((s) => (
                      <option key={s.value} value={s.value} className="bg-muted text-foreground">
                        {s.label}
          ...
          <div className="min-w-0">
            ...
            <input
              ...
              onBlur={(e) => {
                const v = e.target.value || null
                if (v !== item.title) {
                  patch(item.id, 'title', v)
                  persist(item.id, 'title', v)
                }
              }}
              placeholder="(sin título)"
        ...
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  persist(item.id, 'status', e.target.value)
                }}
                className={`w-full text-sm rounded px-3 py-2 focus:outline-none ${STATUS_COLORS[item.status] || 'border border-border bg-muted text-foreground'}`}
              >
                {STATUSES.map((s) => (
                  <option key={s.value} value={s.value} className="bg-muted text-foreground">
                    {s.label}
            ...
            <div>
              ...
              <input
                ...
                onBlur={(e) => {
                  const v = e.target.value || null
                  if (v !== item.publish_date) {
                    patch(item.id, 'publish_date', v)
                    persist(item.id, 'publish_date', v)
                  }
                }}
                className={cls}
              />
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                const v = e.target.value || null
                patch(item.id, 'assigned_to', v)
                persist(item.id, 'assigned_to', v)
              }}
              className={cls}
            >
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                const v = e.target.value || null
                patch(item.id, 'testimonio_id', v)
                persist(item.id, 'testimonio_id', v)
              }}
              className={cls}
            >
                ...
                <div className="flex gap-3">
                  {testimonio.photoUrl && (
                    ...
                    <img
                      ...
                      alt={testimonio.name}
                      className="w-28 h-20 object-cover rounded border border-border shrink-0"
                    />
                  )}
