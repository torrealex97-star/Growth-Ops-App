...
function matchesOnbView(row: StudentRow, view: OnbView): boolean {
  switch (view) {
    ...
    case 'sin_click':
      return (
        !!row.accesos_enviados_at && !row.accesos_abiertos_at && !row.onboarding_scheduled_at && !row.onboarding_date
      )
    case 'click':
      return !!row.accesos_abiertos_at
    case 'agendado':
      return !!row.onboarding_scheduled_at
    case 'onboarded':
      return !!row.onboarding_date
  }
}
...
export default function StudentsPage() {
  ...
  const updateContact = async (
    contactId: string,
    patch: Partial<Pick<StudentContact, 'engagement_score' | 'nps' | 'promise_fulfilled' | 'ttfv_date'>>
  ) => {
    setRows((prev) =>
      prev.map((r) => (r.contacts?.id === contactId ? { ...r, contacts: { ...r.contacts!, ...patch } } : r))
    )
  ...
  const setCourseAccess = async (saleId: string, action: 'grant' | 'revoke') => {
    ...
    setRows((prev) =>
      prev.map((r) =>
        r.id === saleId
          ? {
              ...r,
              course_access_granted_at: action === 'grant' ? now : r.course_access_granted_at,
              course_access_revoked_at: action === 'revoke' ? now : null,
            }
          : r
      )
    )
    ...
    if (!res.ok) {
      toast.error(action === 'grant' ? 'No se pudo conceder el acceso' : 'No se pudo revocar el acceso', {
        description: d?.error,
      })
      return
    }
  ...
  const updateSale = async (
    saleId: string,
    patch: Partial<Pick<StudentRow, 'onboarding_date' | 'first_coaching_date' | 'graduation_date'>>
  ) => {
  ...
  const periodRows = useMemo(() => rows.filter((r) => inPeriod(r.sale_date, range)), [rows, range])
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Journey del alumno — onboarding, coaching, graduación, engagement
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
        ...
        <div className="bg-card border border-border rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-foreground mt-1">
            {kpis.npsAvg !== null ? kpis.npsAvg.toFixed(1) : '—'}
          </p>
        </div>
        <div className="bg-card border border-border rounded-lg p-4">
      ...
      <SearchBox
        value={q}
        onChange={setQ}
        placeholder="Buscar alumno por nombre, email o programa..."
      ...
      <div className="flex flex-wrap gap-2">
        {(
          ...
          ] as { v: OnbView; label: string; n: number }[]
        ).map((o) => (
          <button
            key={o.v}
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {ENGAGEMENT.map((e) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-medium border ${filter === e.value ? 'bg-brand-600 text-white border-brand-600' : 'bg-card text-muted-foreground border-border'}`}
          >
                ...
                return (
                  <>
                    <tr key={r.id} className="border-b border-border/50 last:border-0 hover:bg-card/50">
                      ...
                      <td className="p-3">
                        <span
                          className={`inline-block text-xs px-2 py-1 rounded-md border ${JOURNEY_META[stage].color}`}
                        >
                        ...
                        {progress?.totalMonths && progress.currentMonth ? (
                          <div className="min-w-[110px]">
                            <div className="flex items-center justify-between text-xs text-muted-foreground mb-1">
                              <span>
                                Mes {Math.min(progress.currentMonth, progress.totalMonths)} de {progress.totalMonths}
                              </span>
                            </div>
                            <div className="h-1.5 w-full bg-muted rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${Math.min(100, Math.round((progress.currentMonth / progress.totalMonths) * 100))}%`,
                                }}
                              />
                            </div>
                      ...
                      <td className="p-3">
                        <select
                          ...
                          onChange={(e) =>
                            ...
                            updateContact(contact.id, {
                              engagement_score: (e.target.value || null) as EngagementScore | null,
                            })
                          }
                        ...
                        >
                          ...
                          {ENGAGEMENT.map((e) => (
                            <option key={e.value} value={e.value} className="bg-card text-foreground">
                              {e.label}
                            </option>
                          ))}
                        </select>
                      ...
                      <td className="p-3 text-center">
                        <input
                          ...
                          onChange={(e) =>
                            contact &&
                            updateContact(contact.id, { nps: e.target.value === '' ? null : Number(e.target.value) })
                          }
                      ...
                      <td className="p-3">
                        <select
                          ...
                          onChange={(e) =>
                            contact &&
                            updateContact(contact.id, {
                              promise_fulfilled: (e.target.value || null) as PromiseFulfilled | null,
                            })
                          }
                        ...
                        <td colSpan={12} className="p-4">
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <div>
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  contact && updateContact(contact.id, { ttfv_date: e.target.value || null })
                                }
                                className={`${cls} w-full`}
                              />
                            </div>
                          ...
                          <div className="flex flex-wrap items-center gap-4 mt-4 pt-3 border-t border-border/50">
                            ...
                            <div>
                              ...
                              {r.course_access_revoked_at &&
                              (!r.course_access_granted_at ||
                                r.course_access_revoked_at > r.course_access_granted_at) ? (
                                <div className="flex items-center gap-2">
                                  <span className="text-xs text-red-400">
                                    Revocado {formatDate(r.course_access_revoked_at)}
                                  </span>
                                  <button
                                ...
                                <div className="flex items-center gap-2">
                                  <span className="text-xs text-emerald-400">
                                    Concedido {formatDate(r.course_access_granted_at)}
                                  </span>
                                  <button
                                    disabled={accessSavingId === r.id}
                          ...
                          <div className="text-xs text-muted-foreground mt-3">
                            Fecha de venta: {formatDate(r.sale_date)}
                          </div>
                        </td>
                      </tr>
                    )}
