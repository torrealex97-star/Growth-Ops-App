...
import { Plus, Palette, LayoutGrid, FileImage, Copy, Trash2, Loader2, Sparkles, Bookmark, Trophy } from 'lucide-react'
...
export default function CarruselesDashboard() {
  ...
  const load = useCallback(async () => {
    try {
      const [pRes, tRes] = await Promise.all([
        fetch(`/api/${tenant}/evergreen/carruseles`),
        fetch(`/api/${tenant}/evergreen/carruseles/templates`),
      ])
  ...
  const duplicate = async (id: string, e: React.MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
    const res = await fetch(`/api/${tenant}/evergreen/carruseles/${id}/duplicate`, { method: 'POST' })
    if (res.ok) {
  ...
  const remove = async (id: string, e: React.MouseEvent) => {
    ...
    const res = await fetch(`/api/${tenant}/evergreen/carruseles/${id}`, { method: 'DELETE' })
    if (res.ok) {
  ...
  const deleteTemplate = async (id: string, e: React.MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
    const res = await fetch(`/api/${tenant}/evergreen/carruseles/templates/${id}`, { method: 'DELETE' })
    ...
    >
      <div className="aspect-[4/5] bg-background/40 relative overflow-hidden">
        ...
        <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            onClick={(e) => duplicate(p.id, e)}
            className="h-7 w-7 rounded-md bg-card/90 border border-border flex items-center justify-center text-muted-foreground hover:text-foreground"
            title="Duplicar"
          >
          ...
          <button
            ...
            className="h-7 w-7 rounded-md bg-card/90 border border-border flex items-center justify-center text-muted-foreground hover:text-destructive"
            title="Eliminar"
          >
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            className="gap-1.5 border-brand-500/40 text-brand-300 hover:text-brand-200"
          >
        ...
        <>
          ...
          <div className="mb-10">
            ...
            {casos.length === 0 ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">{casos.map(projectCard)}</div>
            )}
          </div>
          ...
          {projects.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">{others.map(projectCard)}</div>
          )}
                  ...
                  >
                    <div className="aspect-[4/5] bg-background/40 relative overflow-hidden">
                      ...
                      <button
                        ...
                        className="absolute top-2 right-2 h-7 w-7 rounded-md bg-card/90 border border-border flex items-center justify-center text-muted-foreground hover:text-destructive opacity-0 group-hover:opacity-100 transition-opacity"
