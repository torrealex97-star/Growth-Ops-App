...
export default function ConversacionesTab() {
  ...
  useEffect(() => {
    if (platform !== "instagram") { setConfigured(false); setConversations([]); setError(""); return }
    let cancel = false
    ...
    fetch(`/api/${tenant}/evergreen/setting-ai/conversations?platform=instagram`)
      ...
      .then((j) => {
        if (cancel) return
        if (j.error) { setError(j.error); setConfigured(false); return }
        setConfigured(!!j.configured)
        setConversations(j.conversations || [])
      })
      .catch((e) => { if (!cancel) { setError(e.message); setConfigured(false) } })
    ...
    <div className="flex flex-col h-[calc(100vh-6.5rem)] text-foreground">
      <div className="flex items-center gap-3 flex-wrap pb-3 border-b border-border">
        <div>
          ...
          <p className="text-[11px] text-muted-foreground leading-tight">Extrae y analiza con IA las conversaciones reales de redes sociales.</p>
        </div>
        ...
        {platform !== "instagram" ? (
          ...
          <div className="text-center py-14 text-muted-foreground text-sm max-w-md mx-auto">
            ...
            <p>
              ...
              <code className="text-[11px] bg-muted px-1 py-0.5 rounded">instagram_manage_messages</code>) para poder ver y
            ...
            {conversations.map((c) => (
              <div key={c.id} className="border border-border rounded-xl overflow-hidden">
                <button onClick={() => setOpenId(openId === c.id ? null : c.id)}
                  ...
                  {openId === c.id ? <ChevronDown className="w-4 h-4 shrink-0" /> : <ChevronRight className="w-4 h-4 shrink-0" />}
                  <span className="font-medium text-sm flex-1 truncate">{c.participant || "Lead sin nombre"}</span>
                  {c.unread_count > 0 && (
                    <span className="text-[10px] bg-brand-600 text-white rounded-full px-1.5 py-0.5">{c.unread_count} sin leer</span>
                  )}
                ...
                {openId === c.id && (
                  <div className="border-t border-border p-3 bg-background">
                    {c.messages.length === 0 ? (
                      <p className="text-muted-foreground text-xs mb-3">No se pudo extraer la transcripción de esta conversación.</p>
                    ) : (
                    ...
                    <button onClick={() => analyze(c)} disabled={analyzing === c.id || c.messages.length === 0}
...
function PlaceholderPlatform({ platform }: { platform: "facebook" | "tiktok" }) {
  ...
  return (
    <div className="text-center py-14 text-muted-foreground text-sm max-w-md mx-auto">
      ...
      <p>Todavía no está conectada la extracción de conversaciones de {label}. En cuanto se active la integración, aparecerán aquí con el mismo análisis IA que Instagram.</p>
    </div>
  )
}
  ...
  return (
    <div className="mt-3 border border-border rounded-lg p-3 bg-muted/40 text-xs flex flex-col gap-2">
      <div className="flex gap-3 flex-wrap">
        {a.avatar_detectado && <span><b className="text-foreground">Avatar:</b> {a.avatar_detectado}</span>}
        {a.fase_alcanzada && <span><b className="text-foreground">Fase:</b> {a.fase_alcanzada}</span>}
      </div>
      {a.resumen && <p className="text-muted-foreground">{a.resumen}</p>}
      {!!a.fortalezas?.length && (
        <div>
          <b className="text-emerald-400">Fortalezas</b>
          <ul className="list-disc pl-4 mt-0.5">{a.fortalezas.map((f, i) => <li key={i}>{f}</li>)}</ul>
        </div>
      )}
      {!!a.fallos?.length && (
        <div>
          ...
          <ul className="list-disc pl-4 mt-0.5">
            {a.fallos.map((f, i) => <li key={i}><i>&ldquo;{f.cita}&rdquo;</i> — {f.problema}</li>)}
          </ul>
        </div>
      )}
      {!!a.recomendaciones?.length && (
        <div>
          ...
          <ul className="list-disc pl-4 mt-0.5">{a.recomendaciones.map((r, i) => <li key={i}>{r}</li>)}</ul>
        </div>
      )}
    </div>
  )
