...
export default function UsersPage() {
  ...
  const handleRegenerateAllTrackingCodes = async () => {
    ...
    try {
      const res = await fetch(`/api/${tenant}/evergreen/admin/regenerate-tracking-codes`, { method: 'POST' })
  ...
  const handleInvite = async () => {
    ...
    if (sendContract && data.userId && contractTerms) {
      const cRes = await fetch(`/api/${tenant}/evergreen/contracts/team`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId: data.userId,
  ...
  const handleResetPassword = async (user: UserWithRole) => {
    ...
    const res = await fetch(`/api/${tenant}/evergreen/reset-password`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleEditUser = async () => {
    ...
    if (companyChanged || personalChanged) {
      const emRes = await fetch(`/api/${tenant}/evergreen/users`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
