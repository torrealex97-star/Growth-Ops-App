...
import { Users, PlayCircle, AtSign, ExternalLink, Columns3, MessageSquarePlus, StickyNote, Ban, UserCheck, UserPlus } from 'lucide-react'
...
export default function LeadsPage() {
  ...
  const update = async (id: string, patch: Partial<Pick<LeadRow, 'lead_status' | 'lead_channel'>>) => {
    ...
    const res = await fetch(`/api/evergreen/contacts/${id}`, {
      method: 'PATCH',
  ...
  const createContact = async (formData: ContactFormData) => {
    const res = await fetch('/api/evergreen/contacts/create', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2"><Users className="w-6 h-6 text-brand-400" /> Leads</h1>
          <p className="text-muted-foreground text-sm mt-1">Registrados en el formulario del VSL — estado, % visto y seguimiento</p>
        </div>
              ...
              <Fragment key={l.id}>
                <tr className="border-b border-border/50 last:border-0 hover:bg-card/50 align-top">
                  ...
                  <td className="p-3 text-right whitespace-nowrap">
                    <Link href={`/evergreen/contacts/${l.id}`} className="text-brand-400 hover:text-brand-300 inline-flex items-center gap-1 text-xs">
