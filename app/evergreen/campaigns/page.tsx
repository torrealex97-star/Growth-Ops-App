...
export default function CampaignsPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res = await fetch(`/api/evergreen/meta/spend-range?from=${rangeFrom}&to=${rangeTo}`)
  ...
  const runSync = async () => {
    setSyncing(true)
    try {
      const res = await fetch('/api/evergreen/meta/sync', { method: 'POST' })
      const json = await res.json()
      ...
      fetch('/api/evergreen/meta/daily-sync', { method: 'POST' }).catch(() => {})
    } catch (e) {
  ...
  const runDailySync = async () => {
    ...
    try {
      const res = await fetch('/api/evergreen/meta/daily-sync', { method: 'POST' })
      const json = await res.json()
      ...
      if (periodPreset !== 'all' && range.from && range.to) {
        const r = await fetch(`/api/evergreen/meta/spend-range?from=${ymdLocal(range.from)}&to=${ymdLocal(range.to)}`)
  ...
  const runMigrate = async () => {
    setMigrating(true)
    try {
      const res = await fetch('/api/evergreen/admin/migrate-meta', { method: 'POST' })
      const json = await res.json()
  ...
  const runCronSetup = async () => {
    setCroning(true)
    try {
      const res = await fetch('/api/evergreen/admin/setup-meta-cron', { method: 'POST' })
      const json = await res.json()
  ...
  const runAdsSync = async () => {
    setSyncingAds(true)
    try {
      const res = await fetch('/api/evergreen/meta/ads-sync', { method: 'POST' })
      const json = await res.json()
