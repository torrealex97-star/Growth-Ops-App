...
export default function RegistroAfiliadoPage() {
  ...
  useEffect(() => {
    // Enlace por campaña: /evergreen/afiliados/registro?c=<slug>
    const slug = new URLSearchParams(window.location.search).get('c')?.trim() ?? ''
    setCampaignSlug(slug)
    const url = slug ? `/api/evergreen/afiliados/form-config?c=${encodeURIComponent(slug)}` : '/api/evergreen/afiliados/form-config'
    fetch(url)
      .then((r) => r.json())
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      const res = await fetch('/api/evergreen/afiliados/registro', {
        method: 'POST',
    ...
    <div className="dark min-h-screen bg-background flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="flex items-center justify-center gap-2 mb-6">
          ...
          <span className="text-foreground font-semibold text-lg">IA WINNERS</span>
        </div>
