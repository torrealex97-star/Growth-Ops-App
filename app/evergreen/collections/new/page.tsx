...
export default function NewCollectionPage() {
  ...
  const commissionableAmount = selectedSale && grossAmount
    ? parseFloat(grossAmount) * (selectedSale.payment_plans?.cash_collection_ratio ?? 1)
    : 0

  const feePercent = selectedSale?.payment_plans?.fee_percent ?? 0
  const processingFee = selectedSale && grossAmount
    ? parseFloat(grossAmount) * (feePercent / 100)
    : 0
  const netCashCollected = selectedSale && grossAmount
    ? parseFloat(grossAmount) - processingFee
    : 0
  ...
  const previewCommissions = selectedSale && grossAmount && !needsCommissionReview
  ...
  const handleSubmit = async () => {
    ...
    router.push('/evergreen/collections')
  }
      ...
      <div className="bg-card border border-border rounded-lg p-6 space-y-5">
          ...
          {selectedSale ? (
            <div className="bg-muted border border-border rounded-lg p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{selectedSale.payment_plans?.name} — {formatCurrency(selectedSale.gross_amount)}</p>
                <p className="text-xs text-muted-foreground">Plazo devolucion: {formatDate(selectedSale.refund_deadline_at)}</p>
              </div>
                ...
                <div className="absolute top-full left-0 right-0 z-10 mt-1 border border-border rounded-lg overflow-hidden bg-card">
                  {searchLoading ? (
                    ...
                    >
                      ...
                      <p className="text-muted-foreground text-xs">{s.payment_plans?.name} — {formatCurrency(s.gross_amount)}</p>
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="rounded-lg p-4 border flex items-start gap-3 bg-blue-500/10 border-blue-500/30">
            ...
            <div>
              ...
              <p className="text-xs text-blue-300/70 mt-0.5">
                Esta venta ya tiene un cobro comisionable (reserva/entrada). Este cobro se registrará
                pero NO generará comisión automáticamente: quedará en revisión de pagos hasta que el
                equipo lo apruebe manualmente.
              </p>
            </div>
          </div>
        )}
            ...
            {isEligible ? (
              <>
                ...
                <div>
                  ...
                  <p className="text-xs text-emerald-300/70 mt-0.5">El plazo de devolucion ya ha pasado. Se generaran comisiones.</p>
                </div>
              </>
            ) : (
              <>
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-300/70 mt-0.5">
                    Quedan {daysUntilEligible} dias para el plazo de devolucion ({formatDate(refundDeadline?.toISOString())}).
                  </p>
                </div>
              </>
            )}
            ...
            <div className="space-y-2">
              {previewCommissions.map((com, i) => (
                <div key={i} className="flex justify-between text-sm">
                  <span className="text-muted-foreground capitalize">{com.participant_type} ({com.percent}%)</span>
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button
            ...
            disabled={submitting || !selectedSale || !grossAmount || !paymentMethod}
          >
            {submitting ? (
              <>
