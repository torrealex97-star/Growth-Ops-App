...
export default function InstagramPage() {
  ...
  const runSync = async () => {
    setSyncing(true)
    try {
      const res = await fetch('/api/evergreen/instagram/sync', { method: 'POST' })
  ...
  const runMigrate = async () => {
    setMigrating(true)
    try {
      const res = await fetch('/api/evergreen/admin/migrate-instagram', { method: 'POST' })
      const json = await res.json()
      if (!res.ok) throw new Error(json.error)
  ...
  const runCron = async () => {
    setCroning(true)
    try {
      const res = await fetch('/api/evergreen/admin/setup-instagram-cron', { method: 'POST' })
      const json = await res.json()
      if (!res.ok) throw new Error(json.error)
  ...
  const transcribe = async (id: string) => {
    setBusyId(id)
    try {
      const res = await fetch('/api/evergreen/instagram/transcribe', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ mediaId: id }) })
      const json = await res.json()
      if (!res.ok) throw new Error(json.error)
  ...
  const genScript = async (id: string, saveAsIdea: boolean) => {
    ...
    try {
      const res = await fetch('/api/evergreen/instagram/script', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ mediaId: id, saveAsIdea, testimonio: testimonioPick || undefined }) })
      const json = await res.json()
      if (!res.ok) throw new Error(json.error)
