...
export default function ReelsDelDiaPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/evergreen/reels?${params.toString()}`)
  ...
  const patch = async (id: string, body: Record<string, unknown>) => {
    setBusyFor(id, true)
    try {
      const res = await fetch(`/api/evergreen/reels/${id}`, {
  ...
  const goToCarousel = (d: Draft) => {
    ...
    router.push(`/evergreen/carruseles${idea ? `?idea=${encodeURIComponent(idea)}` : ''}`)
  }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            ...
            (hook + CTA [tenant]), la transcripción original y una idea de carrusel. Tú decides qué aprobar.
          </p>
        </div>
                ...
                <div className="p-4 space-y-3 flex-1">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <label className="text-xs text-muted-foreground">Guión adaptado (hook + CTA [tenant])</label>
                    ...
                    {testimonio && (
                      <div className="mt-2 rounded-lg border border-pink-500/25 bg-pink-950/10 p-3">
                        ...
                        <div className="flex flex-wrap items-center gap-2 mt-2.5">
                          <Link
                            href={`/evergreen/testimonios/${testimonio.id}`}
