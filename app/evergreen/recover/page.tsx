...
export default function RecoverPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/evergreen/auth/recover', {
        method: 'POST',
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: `${window.location.origin}/api/evergreen/auth/callback?next=/evergreen/settings/password`,
    })
    ...
    <div className="dark min-h-screen bg-background flex items-center justify-center p-4" data-theme="os">
      <div className="w-full max-w-sm">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-foreground">IA WINNERS</h1>
