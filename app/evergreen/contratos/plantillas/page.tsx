...
export default function PlantillasPage() {
  ...
  const openNew = () => {
    setEditing(null); setName(''); setKind('alumno'); setPaymentMethod(''); setWelcome(''); setRoleKey(''); setBody(''); setDialog(true)
  }
  const openEdit = (t: ContractTemplate) => {
    setEditing(t); setName(t.name); setKind((t.kind as 'equipo' | 'alumno' | 'tomador' | 'colaborador') ?? 'alumno')
    setPaymentMethod(t.payment_method ?? ''); setWelcome(t.welcome_message ?? '')
    setRoleKey(t.role_key ?? ''); setBody(t.body); setDialog(true)
  }
  ...
  const generateWithAI = async () => {
    if (!body.trim()) { toast.error('Pega primero el texto del contrato'); return }
    setAiLoading(true)
    const res = await fetch('/api/evergreen/contracts/templates/ai', {
  ...
  const save = async () => {
    if (!name.trim() || !body.trim()) { toast.error('Nombre y cuerpo son obligatorios'); return }
    setSaving(true)
    const sb = createClient()
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Crea plantillas por tipo (alumno / tomador / equipo) y asócialas a un método de pago: al generar la venta se enviará la plantilla del método seleccionado.</p>
        </div>
        <Button onClick={openNew}><Plus className="w-4 h-4 mr-2" />Nueva plantilla</Button>
      </div>
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    <Badge className="border text-xs bg-brand-700/30 text-brand-300 border-brand-600/40">{kindLabel(t.kind)}</Badge>
                    {(t.kind === 'alumno' || t.kind === 'tomador') && (
                      <Badge className="border text-xs bg-muted/40 text-foreground border-border">{methodLabel(t.payment_method)}</Badge>
                    )}
                    {t.role_key && <Badge className="border text-xs bg-muted/40 text-foreground border-border">{t.role_key}</Badge>}
                  </div>
                </div>
                <div className="flex gap-1">
                  <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-foreground" onClick={() => openEdit(t)}><Edit2 className="w-3.5 h-3.5" /></Button>
                  <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-red-400" onClick={() => remove(t)}><Trash2 className="w-3.5 h-3.5" /></Button>
                </div>
              </div>
          ...
          <div className="space-y-4 mt-2">
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label>Nombre</Label>
                <Input value={name} onChange={(e) => setName(e.target.value)} className="bg-muted border-border" placeholder="Contrato alumno — Sequra" />
              </div>
              <div className="space-y-2">
                <Label>Tipo de plantilla</Label>
                <Select value={kind} onValueChange={(v) => setKind(v as 'equipo' | 'alumno' | 'tomador' | 'colaborador')}>
            ...
            {isStudentLike && (
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={paymentMethod || '__default'} onValueChange={(v) => setPaymentMethod(v === '__default' ? '' : v)}>
                    <SelectTrigger className="bg-muted border-border"><SelectValue /></SelectTrigger>
                    <SelectContent className="bg-card border-border">
                      {PAYMENT_METHODS.map((m) => <SelectItem key={m.value || '__default'} value={m.value || '__default'}>{m.label}</SelectItem>)}
                    </SelectContent>
                  </Select>
                  <p className="text-[11px] text-muted-foreground">Se usará esta plantilla cuando la venta tenga ese método. &ldquo;Por defecto&rdquo; cubre los métodos sin plantilla propia.</p>
                </div>
                <div className="space-y-2">
                  <Label>Mensaje de bienvenida (email)</Label>
                  <Textarea value={welcome} onChange={(e) => setWelcome(e.target.value)} className="bg-muted border-border min-h-[70px] text-sm" placeholder="¡Bienvenido Winner! Revisa y acepta las condiciones…" />
                </div>
              </div>
            )}
            ...
            {(kind === 'equipo' || kind === 'colaborador') && (
              <div className="space-y-2">
                ...
                <Input value={roleKey} onChange={(e) => setRoleKey(e.target.value)} className="bg-muted border-border" placeholder="setter / closer / affiliate" />
              </div>
            )}
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Button type="button" variant="outline" size="sm" onClick={generateWithAI} disabled={aiLoading}>
                  {aiLoading ? <><Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" />Generando…</> : <><Sparkles className="w-3.5 h-3.5 mr-1.5" />Insertar variables con IA</>}
                </Button>
              </div>
              <Textarea ref={bodyRef} value={body} onChange={(e) => setBody(e.target.value)} className="bg-muted border-border min-h-[280px] font-mono text-xs" placeholder="Pega aquí el texto del contrato… luego pulsa 'Insertar variables con IA' o inserta variables con los botones de abajo." />
              <div className="flex flex-wrap gap-1.5">
                {VARS.map(({ v, help }) => (
                  <button key={v} type="button" onClick={() => insertVar(v)} title={help}
              ...
              <p className="text-xs text-muted-foreground">Las condiciones económicas y las firmas se añaden automáticamente al final del contrato.</p>
            </div>

            <div className="flex justify-end gap-3 pt-2">
              <Button variant="outline" onClick={() => setDialog(false)} disabled={saving}>Cancelar</Button>
              <Button onClick={save} disabled={saving}>{saving ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Guardando…</> : 'Guardar'}</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
