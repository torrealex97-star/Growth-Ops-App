...
export default function ContratosEquipoPage() {
  ...
  const [result, setResult] = useState<{ name: string; url: string; emailed: boolean; emailError: string | null; email: string | null; personalEmail: string | null } | null>(null)
  ...
  const onSelectUser = (id: string) => {
    ...
    if (!u) { setTerms(null); setPersonalEmail(''); return }
  ...
  const generate = async () => {
    if (!userId || !terms) { toast.error('Selecciona un miembro'); return }
    setGenerating(true)
    const res = await fetch('/api/evergreen/contracts/team', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userId, templateId: templateId || null, title, terms, roleKey, roleLabel: roleKey ? ROLE_LABELS[roleKey] : null, personalEmail: personalEmail.trim() || null }),
    })
    const d = await res.json()
    setGenerating(false)
    if (!res.ok) { toast.error('No se pudo generar', { description: d.error }); return }
    setDialog(false)
    setResult({ name: selectedUser?.full_name ?? 'el colaborador', url: d.signUrl, emailed: !!d.emailed, emailError: d.emailError ?? null, email: d.memberEmail ?? null, personalEmail: d.personalEmail ?? null })
    setUserId(''); setRoleKey(''); setTerms(null); setTitle(''); setTemplateId(''); setPersonalEmail('')
    load()
  }

  const copyLink = (url: string) => { navigator.clipboard?.writeText(url); toast.success('Enlace copiado') }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Genera y envía contratos a firmar con las condiciones del miembro (fijo + comisiones).</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => (window.location.href = '/evergreen/contratos/plantillas')}>Plantillas</Button>
          <Button onClick={() => setDialog(true)}><Plus className="w-4 h-4 mr-2" />Nuevo contrato</Button>
        </div>
      </div>
            ...
            <div key={c.id} className="flex items-center justify-between gap-4 p-4 bg-card/40">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                {c.status !== 'firmado' && c.signing_token && (
                  <Button variant="ghost" size="sm" className="text-muted-foreground hover:text-foreground" onClick={() => copyLink(`${window.location.origin}/firmar/${c.signing_token}`)}>
                    <Copy className="w-3.5 h-3.5 mr-1" /> Enlace
                  </Button>
                )}
                {c.signed_pdf_url && (
                  <a href={c.signed_pdf_url} target="_blank" rel="noopener noreferrer" className="text-sm text-emerald-400 hover:underline flex items-center gap-1">
                    PDF <ExternalLink className="w-3.5 h-3.5" />
                  </a>
                )}
                <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-red-400" onClick={() => del(c)}><Trash2 className="w-3.5 h-3.5" /></Button>
              </div>
            </div>
          ))}
        </div>
      ...
      <Dialog open={dialog} onOpenChange={setDialog}>
        <DialogContent className="bg-card border-border text-foreground max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>Nuevo contrato de equipo</DialogTitle></DialogHeader>
          <div className="space-y-4 mt-2">
            <div className="space-y-2">
              <Label>Miembro del equipo</Label>
              <Select value={userId} onValueChange={onSelectUser}>
                <SelectTrigger className="bg-muted border-border"><SelectValue placeholder="Seleccionar miembro…" /></SelectTrigger>
                <SelectContent className="bg-card border-border">
                  {users.map((u) => <SelectItem key={u.id} value={u.id}>{u.full_name} · {u.roles?.name}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
              ...
              <>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Se añade al contrato como variable <code className="text-muted-foreground">{'{{email_personal}}'}</code> y recibe una copia.
                    El contrato se envía a <span className="text-foreground">{selectedUser?.email || 'su correo de empresa'}</span> y, si lo indicas, con copia al personal.
                  </p>
                </div>
              </>
            )}
            ...
            <div className="flex justify-end gap-3 pt-2">
              <Button variant="outline" onClick={() => setDialog(false)} disabled={generating}>Cancelar</Button>
              <Button onClick={generate} disabled={generating || !userId || !terms}>
                {generating ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Generando…</> : 'Generar y obtener enlace'}
              </Button>
            </div>
          </div>
        </DialogContent>
      ...
      <Dialog open={!!result} onOpenChange={(o) => !o && setResult(null)}>
        <DialogContent className="bg-card border-border text-foreground max-w-md">
          <DialogHeader><DialogTitle>Contrato listo para firmar</DialogTitle></DialogHeader>
          <div className="space-y-4 mt-2">
            {result?.emailed ? (
              <div className="rounded-lg bg-emerald-500/10 border border-emerald-500/30 p-3 text-sm text-emerald-300">
                ✓ Email enviado automáticamente a <span className="text-foreground">{result.email ?? result.personalEmail}</span>
                {result.personalEmail && result.email && <> con copia a <span className="text-foreground">{result.personalEmail}</span></>}. También puedes compartir el enlace manualmente:
              </div>
            ) : (
              <div className="rounded-lg bg-amber-500/10 border border-amber-500/30 p-3 text-xs text-amber-300">
                {result?.emailError
                  ...
                  : <>Envío por email no configurado. Comparte este enlace con <span className="text-foreground">{result?.name}</span> (WhatsApp, email…):</>}
              </div>
            )}
            <p className="text-sm text-muted-foreground">Al firmarlo, el PDF se guarda automáticamente aquí y se puede descargar.</p>
            <div className="flex items-center gap-2">
              <code className="flex-1 bg-muted border border-border rounded-lg px-3 py-2.5 text-xs text-sky-400 font-mono break-all select-all">{result?.url}</code>
              <Button variant="outline" size="icon" onClick={() => result && copyLink(result.url)} title="Copiar"><Copy className="w-4 h-4" /></Button>
            </div>
            <div className="flex justify-end"><Button onClick={() => setResult(null)}>Hecho</Button></div>
          </div>
        </DialogContent>
      </Dialog>
    </div>
