  ...
  const handleSubmit = async () => {
    ...
    if (proofFile) {
      try {
        ...
        const b64 = btoa(bin)
        const res = await fetch('/api/evergreen/sales/payment-proof', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    if (reservationId) {
      ...
      const res = await fetch('/api/evergreen/sales/complete-reservation', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    const recordCollection = async (grossAmt: number, commissionableAmt?: number): Promise<boolean> => {
      ...
      try {
        const res = await fetch('/api/evergreen/collections/record', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
    ...
    try {
      const cRes = await fetch('/api/evergreen/contracts/student', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    router.push(`/evergreen/sales/${saleId}`)
  }
