...
export default function SaleDetailPage({ params }: { params: { id: string } }) {
  ...
  const fetchFollowUps = useCallback(async () => {
    ...
    const res = await fetch(`/api/evergreen/sales/${id}/follow-ups`)
  ...
  const handleEditSubmit = async () => {
    ...
    const res = await fetch('/api/evergreen/sales/update', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        saleId: id,
  ...
  const markInstallmentPaid = async (installment: SaleExpectedInstallment) => {
    ...
    const res = await fetch('/api/evergreen/payments/mark', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleEditCollectionSubmit = async () => {
    if (!editColl) return
    setCollSaving(true)
    const res = await fetch(`/api/evergreen/collections/${editColl.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  const handleDeleteCollection = async () => {
    ...
    const res = await fetch(`/api/evergreen/collections/${deleteColl.id}`, { method: 'DELETE' })
    const data = await res.json().catch(() => ({}))
  ...
  const handleRefundSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/evergreen/refunds/create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          saleId: id,
      ...
      <div className="flex items-start justify-between">
        ...
        {isReservation && (
          <Button
            ...
            onClick={() => router.push(`/evergreen/sales/new?contact=${sale.contact_id}&reserva=${sale.reservation_amount ?? sale.gross_amount}&product=${sale.product_id}&reservationId=${sale.id}`)}
          >
      ...
      <Tabs defaultValue="detail">
        ...
        <TabsContent value="collections" className="mt-4">
          <div className="flex justify-between items-center mb-4">
            ...
            <Button size="sm" onClick={() => router.push(`/evergreen/collections/new?saleId=${sale.id}`)}>
              ...
              <div className="space-y-1.5">
                ...
                <Select value={editForm.closer_id} onValueChange={(v) => setEditForm((f) => ({ ...f, closer_id: v }))}>
                  ...
                  <SelectContent className="bg-card border-border">
                    ...
                    {/* Incluye admin: hay admins (ej. Jesús Peña) que también cierran ventas y deben poder marcarse como closer. */}
