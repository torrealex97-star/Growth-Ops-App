...
export default function CarruselEditorPage({ params }: { params: { id: string } }) {
  ...
  const fetchProject = useCallback(async () => {
    try {
      const res = await fetch(`/api/evergreen/carruseles/${id}`)
  ...
  const handleDeleteSlide = async (slideId: string) => {
    ...
    const res = await fetch(`/api/evergreen/carruseles/${id}/slides/${slideId}`, { method: "DELETE" })
    if (res.ok) fetchProject()
  }

  const handleUndoSlide = async (slideId: string) => {
    const res = await fetch(`/api/evergreen/carruseles/${id}/slides/${slideId}/undo`, { method: "POST" })
    if (res.ok) fetchProject()
  ...
  const handleReorder = async (slideIds: string[]) => {
    ...
    await fetch(`/api/evergreen/carruseles/${id}/slides`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  const saveTemplate = async () => {
    const res = await fetch("/api/evergreen/carruseles/templates", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  if (notFound) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center gap-3">
        ...
        <Link href="/evergreen/carruseles" className="text-sm text-brand-400 underline">
          Volver
        </Link>
      </div>
    )
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-7.5rem)] min-h-[560px] -m-4 lg:-m-6">
      ...
      <div className="h-12 border-b border-border bg-card flex items-center px-3 gap-2 shrink-0">
        <Link href="/evergreen/carruseles" className="h-8 w-8 flex items-center justify-center rounded-md text-muted-foreground hover:text-foreground hover:bg-muted">
