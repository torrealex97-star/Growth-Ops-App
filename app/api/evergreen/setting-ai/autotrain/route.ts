      ...
      try {
        ...
        for (let ci = 0; ci < numConvos; ci++) {
          ...
          for (let t = 0; t < turns; t++) {
            // 1) Lead simulado
            const leadMsgs = ensureStartsUser(toLeadMessages(conversation), '(Acabas de abrir el DM. Escribe tu primer mensaje como lead, breve y natural.)')
            const leadText = await callText({ model: leadModel, system: leadSystem(persona), messages: leadMsgs, max_tokens: 400 })
            ...
            const agentText = await callText({ model: chatModel, system: sys, messages: toAgentMessages(conversation), max_tokens: 700 })
        ...
        if (autoCorr.length) {
          ...
          try {
            improved = await callText({ model: chatModel, system, messages: [{ role: 'user', content: user }], max_tokens: 14000, temperature: 0.4 })
          } catch {
            improved = await callText({ model: DEFAULT_MODEL, system, messages: [{ role: 'user', content: user }], max_tokens: 14000, temperature: 0.4 })
          }
        } else {
          improved = '(El auto-entrenamiento no detectó fallos que corregir. El prompt actual aguantó bien esta tanda 💪)'
        }
