...
import { requireCaller, getClient, modelFrom, DEFAULT_MODEL, buildImprovePrompt, type Correction } from '@/lib/setting-ai/core'
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const runWith = async (m: string) => {
        const ms = client.messages.stream({ model: m, max_tokens: 14000, system, temperature: 0.4, messages: [{ role: 'user', content: user }] })
        ms.on('text', (t: string) => send({ type: 'token', text: t }))
        await ms.finalMessage()
      }
      try {
        try { await runWith(model) }
        catch (e) {
          if (model !== DEFAULT_MODEL) { await runWith(DEFAULT_MODEL) } else { throw e }
        }
        send({ type: 'done' })
      } catch (e) {
