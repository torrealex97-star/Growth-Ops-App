...
export async function POST(req: NextRequest, { params }: { params: Promise<{ tenant: string }> }) {
  try {
    ...
    if (targetId) {
      const { data } = await sb.from('contracts').select('id').eq('id', targetId).eq('tenant_id', tenantId).maybeSingle()
      targetId = data?.id ?? null
    }
    if (!targetId && saleId) {
      const { data } = await sb.from('contracts').select('id').eq('sale_id', saleId).eq('tenant_id', tenantId).order('created_at', { ascending: false }).limit(1).maybeSingle()
      targetId = data?.id ?? null
    }
    if (!targetId && email) {
      const { data: contact } = await sb.from('contacts').select('id').eq('email', email).eq('tenant_id', tenantId).maybeSingle()
      if (contact) {
        const { data } = await sb.from('contracts').select('id').eq('contact_id', contact.id).eq('tenant_id', tenantId).neq('status', 'firmado').order('created_at', { ascending: false }).limit(1).maybeSingle()
        targetId = data?.id ?? null
      }
    }
