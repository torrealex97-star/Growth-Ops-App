...
export async function GET(req: NextRequest) {
  try {
    ...
    const sb = service()
    const cols = 'id, status, signing_token, read_at, signed_at, signed_pdf_url, accesos_enviados_at, accesos_abiertos_at, onboarding_webhook_ok, email_sent_at, is_reservation, contract_party'
    const { data } = await sb
      .from('contracts')
      .select(cols)
      .eq('sale_id', saleId)
    ...
    const salePaymentMethod = (saleRow?.payment_method as string | null)
      ?? ((saleRow?.payment_plans as { method?: string | null } | null)?.method ?? null)
    ...
    return NextResponse.json({ contract, payerContract, hasDistinctPayer, recommendedRecipient, payerName: payer?.name ?? null })
  } catch (err) {
...
export async function POST(req: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          message: 'This sale requires document verification before sending the contract. Please upload and verify identification documents or apply an override.'
        },
        { status: 403 }
      )
    }
    ...
    const terms: StudentContractTerms = {
      product_name: product.name || 'Programa [tenant]',
    ...
    const upsertContract = async (opts: {
      ...
      const title = opts.party === 'tomador'
      ...
      if (existing) {
        ...
        const { data: created, error } = await sb.from('contracts').insert({
          kind: 'venta', sale_id: saleId, contact_id: sale.contact_id,
          template_id: opts.tpl?.id ?? null, title,
    ...
    if (includePayer) {
      ...
      if (send && payerEmail) {
        const rp = await sendStudentContractEmail({
          to: payerEmail, studentName: payer!.name!, company, signUrl: payerContract.signUrl,
