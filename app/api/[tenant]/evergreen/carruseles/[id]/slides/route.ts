...
export async function POST(req: NextRequest, { params }: Ctx) {
  try {
    ...
    if (!slide)
      return NextResponse.json({ error: 'No se pudo añadir (límite o proyecto inexistente)' }, { status: 400 })
    return NextResponse.json(slide)
  } catch (err) {
    console.error('[api/carruseles/id/slides POST]', err)
...
export async function PUT(req: NextRequest, { params }: Ctx) {
  try {
    ...
    console.error('[api/carruseles/id/slides PUT]', err)
    return NextResponse.json({ error: 'Error interno del servidor' }, { status: 500 })
  }
}
