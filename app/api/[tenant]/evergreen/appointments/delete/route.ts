...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: linkedSales } = await sb.from('sales').select('id, status').eq('appointment_id', appointmentId)
    ...
    await sb.from('appointments').update({ origin_appointment_id: null }).eq('origin_appointment_id', appointmentId)

    const { error: delErr } = await sb.from('appointments').delete().eq('id', appointmentId)
