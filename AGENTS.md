...
- **Un test en rojo con arnés equivocado no es una regresión.** Las suites corren SOLO por los scripts de `package.json`: `npm test`, `npm run test:metrics` (necesita el loader de `tests/register.mjs`) y `npm run test:e2e` (los specs de `tests/e2e/*.spec.mjs` son de Playwright, NO de `node:test`). Invocarlos a mano con `node --test` produce fallos falsos (25-sep: 4 "fallos" E2E y 1 de métricas que solo eran del arnés). Antes de tocar código por un test en rojo, verifica que lo lanzaste por el script canónico.
- **Sin merge-base no hay merge (linaje huérfano).** Antes de fusionar una PR o continuar una rama ajena, `git merge-base main <rama>` debe devolver un commit. Si devuelve vacío, NO fusionar NI rebasar: extraer el contenido único (verificado commit a commit), relevarlo en una rama nueva desde `main` y cerrar la PR como sustituida con comentario explicando qué se verificó. Caso #210 (25-sep): fusionar habría revertido el tablero y arrastrado 35 ficheros del linaje viejo.
- **Los presupuestos de un cron en Vercel suman MUY POR DEBAJO de su `maxDuration`.** 35+25 = 60 s exactos murió con 504 `FUNCTION_INVOCATION_TIMEOUT` (25-sep, `cron/calendly-ghl`): antes del primer fetch ya se gastan cold start, middleware, config por subcuenta y candados de `recordSyncRun`. La suma de deadlines deja ~20 s de colchón, y un corte por presupuesto no es un fallo — upsert idempotente, el run siguiente continúa.
