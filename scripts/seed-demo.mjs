...
async function main() {
  ...
  const closers = [await ensureTeam('Laura Gómez', 'laura@demo.closer', 'closer'),
                   await ensureTeam('Marcos Ruiz', 'marcos@demo.closer', 'closer')]
  const setters = [await ensureTeam('Ana Torres', 'ana@demo.closer', 'setter'),
                   await ensureTeam('David Sanz', 'david@demo.closer', 'setter')]

  console.log('→ Productos y planes…')
  const { data: prod } = await sb.from('products').insert({ name: 'Programa Closer Élite (demo)', is_active: true }).select().single()
  const { data: plan } = await sb.from('payment_plans').insert({
    product_id: prod.id, name: 'Pago único (demo)', gross_price: 3000, number_of_payments: 1, cash_collection_ratio: 1.0,
  }).select().single()
  ...
  for (let i = 0; i < 16; i++) {
    const { data: c } = await sb.from('contacts').insert({
      full_name: `Lead Demo ${i + 1}`, email: `lead${i + 1}@demo.closer`, notes: '[DEMO]',
    }).select().single()
  ...
  const months = [[2026, 3], [2026, 4], [2026, 5], [2026, 6]]
  ...
  for (let mi = 0; mi < months.length; mi++) {
    ...
    for (let k = 0; k < salesThisMonth; k++) {
      ...
      const { data: sale } = await sb.from('sales').insert({
        contact_id: contact, product_id: prod.id, payment_plan_id: plan.id,
        sale_date: sd, refund_deadline_at: addDays(sd, 14), gross_amount: gross,
        closer_id: closer, setter_id: setter, created_by: ADMIN_ID,
        status: refunded ? 'refunded' : 'active', notes: '[DEMO]',
      }).select().single()
      if (!refunded) {
        // cobro: 70-100% del bruto
        const cash = n % 3 === 0 ? Math.round(gross * 0.5) : gross
        await sb.from('collections').insert({
          sale_id: sale.id, collected_at: `${sd}T12:00:00Z`, gross_amount: cash,
          commissionable_amount: cash, status: 'collected', is_confirmed: true,
        })
      }
      // agenda asociada
      await sb.from('appointments').insert({
        contact_id: contact, appointment_datetime: `${sd}T10:00:00Z`,
        status: pick(['show', 'show', 'no_show', 'show'], n), setter_id: setter, closer_id: closer,
  ...
  await sb.from('targets').insert([
    { name: 'Facturación mensual', scope_type: 'company', metric_key: 'revenue', period_type: 'monthly', ...jun, target_value: 30000, is_active: true, created_by: ADMIN_ID },
    { name: 'Ventas del mes', scope_type: 'company', metric_key: 'sales_count', period_type: 'monthly', ...jun, target_value: 12, is_active: true, created_by: ADMIN_ID },
    { name: 'Cash collected del mes', scope_type: 'company', metric_key: 'cash_collected', period_type: 'monthly', ...jun, target_value: 20000, is_active: true, created_by: ADMIN_ID },
  ])

  console.log(`✓ Demo sembrada: ${n} ventas, ${contacts.length} contactos, equipo de ${closers.length + setters.length}.`)
}
