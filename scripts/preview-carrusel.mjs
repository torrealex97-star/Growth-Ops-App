...
const t = fs.readFileSync(".env.local", "utf8"); const e = {}
t.split("\n").forEach((l) => { const m = l.match(/^([A-Z0-9_]+)\s*=\s*(.*)$/); if (m) e[m[1]] = m[2].replace(/^["']|["']$/g, "").trim() })
...
data.slides.forEach((s, i) => { fs.writeFileSync(`/tmp/slide_${i}.html`, wrap(s.html)) })
