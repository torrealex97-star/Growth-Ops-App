  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('payment_method', {
        header: 'Metodo',
        cell: ({ getValue }) => <span className="text-muted-foreground text-sm capitalize">{getValue() || '—'}</span>,
      }),
      columnHelper.accessor('is_eligible_for_commission', {
        header: 'Elegible',
        cell: ({ getValue }) =>
          getValue() ? <Badge variant="success">Si</Badge> : <Badge variant="secondary">No</Badge>,
      }),
      columnHelper.accessor('status', {
        ...
        cell: ({ getValue }) => {
          const s = getValue()
          return (
            <Badge variant={s === 'collected' ? 'success' : s === 'reversed' ? 'destructive' : 'warning'}>{s}</Badge>
          )
        },
      }),
    ],
