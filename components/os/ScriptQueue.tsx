...
export function ScriptQueueProvider({ children }: { children: React.ReactNode }) {
  ...
  const runJob = async (job: ScriptJob) => {
    try {
      ...
      const res = await fetch(`/api/${tenant}/evergreen/instagram/script`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          competitorMediaId: job.cmId,
          transcript,
          saveAsIdea: job.save,
          testimonio: job.testimonio || undefined,
        }),
      })
      ...
      if (job.save && json.ideaId) {
        toast.success(`Guión de ${job.label} añadido a Ideas`, {
          action: { label: 'Ver', onClick: () => router.push(`/${tenant}/marketing/contenido`) },
        })
    ...
    while (runningRef.current < CONCURRENCY) {
      ...
      runJob(next).finally(() => {
        ...
        const active = jobsRef.current.some(
          (j) => j.status === 'queued' || j.status === 'transcribing' || j.status === 'scripting'
        )
        if (!active) {
  ...
  useEffect(() => {
    try {
      ...
      const resumed = parsed.map((j) =>
        j.status === 'transcribing' || j.status === 'scripting' ? { ...j, status: 'queued' as JobStatus } : j
      )
  ...
  const retryJob = (cmId: string) => {
    patchJob(cmId, { status: 'queued', error: undefined })
    pump()
  }
  ...
  const saveDraftAsIdea = async (job: ScriptJob) => {
    ...
    const { data, error } = await supabase
      ...
      .single()
    if (error) {
      toast.error('No se pudo añadir a Ideas', { description: error.message })
      return
    }
    patchJob(job.cmId, { save: true, ideaId: data?.id ?? null })
    toast.success('Añadido a Ideas', {
      action: { label: 'Ver', onClick: () => router.push(`/${tenant}/marketing/contenido`) },
    })
    setReviewId(null)
  }
        ...
        <div className="fixed bottom-4 right-4 z-[60] w-80 bg-card border border-border rounded-xl shadow-2xl overflow-hidden">
          <div className="flex items-center justify-between px-3 py-2 border-b border-border bg-card/80">
            ...
            <div className="flex items-center gap-1">
              {active === 0 && (
                <button onClick={clearFinished} className="text-xs text-muted-foreground hover:text-foreground px-1">
                  Limpiar
                </button>
              )}
              <button onClick={() => setTrayOpen((o) => !o)} className="text-muted-foreground hover:text-foreground">
                {trayOpen ? <ChevronDown className="w-4 h-4" /> : <ChevronUp className="w-4 h-4" />}
              </button>
            </div>
          </div>
          {trayOpen && (
              ...
              {jobs.map((j) => (
                <div key={j.cmId} className="flex items-center gap-2 px-3 py-2 text-xs">
                  <span className="shrink-0">
                    ...
                    {(j.status === 'transcribing' || j.status === 'scripting') && (
                      <Loader2 className="w-4 h-4 animate-spin text-pink-400" />
                    )}
                  ...
                  {j.status === 'done' &&
                    (j.save && j.ideaId ? (
                      <button
                        onClick={() => router.push(`/${tenant}/marketing/contenido`)}
                        className="shrink-0 text-pink-300 hover:text-pink-200"
                      >
                        Ver
                      </button>
                    ) : (
                      <button
                        onClick={() => setReviewId(j.cmId)}
                        className="shrink-0 text-pink-300 hover:text-pink-200"
                      >
                        Abrir
                      </button>
            ...
            <div className="space-y-3 text-sm">
              ...
              {review.draft.cta_used && (
                <span className="inline-block text-xs bg-pink-950/50 text-pink-300 px-2 py-0.5 rounded-full">
                  CTA: {review.draft.cta_used}
                </span>
              )}
              <div>
                <span className="text-muted-foreground">Hook:</span>{' '}
                <span className="text-pink-300">{review.draft.hook}</span>
              </div>
              <div>
              ...
              <div>
                ...
                <span className="text-foreground">{review.draft.caption}</span>
              </div>
              <div className="flex flex-wrap gap-2 pt-2">
                <button
                  onClick={() => saveDraftAsIdea(review)}
                  className="px-3 py-2 rounded-lg text-sm bg-pink-600 text-foreground hover:bg-pink-500"
                >
                  Añadir a Contenido como IDEA
                </button>
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(
                      `${review.draft!.hook}\n\n${review.draft!.script}\n\n${review.draft!.caption}`
                    )
