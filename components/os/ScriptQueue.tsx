...
export function ScriptQueueProvider({ children }: { children: React.ReactNode }) {
  ...
  const saveDraftAsIdea = async (job: ScriptJob) => {
    ...
    toast.success('Añadido a Ideas', { action: { label: 'Ver', onClick: () => router.push(`/${tenant}/marketing/contenido`) } })
    setReviewId(null)
  }
            ...
            <div className="max-h-72 overflow-y-auto divide-y divide-border/60">
              {jobs.map((j) => (
                <div key={j.cmId} className="flex items-center gap-2 px-3 py-2 text-xs">
                  ...
                  {j.status === 'done' && (
                    j.save && j.ideaId
                      ? <button onClick={() => router.push(`/${tenant}/marketing/contenido`)} className="shrink-0 text-pink-300 hover:text-pink-200">Ver</button>
