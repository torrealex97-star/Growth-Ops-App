...
const COLS: { key: keyof DailyFunnelRow; label: string; fmt: (r: DailyFunnelRow) => string; hint?: string }[] = [
  ...
  { key: 'tasaConversionVSL', label: '% conversión VSL', fmt: (r) => fmtPct(r.tasaConversionVSL), hint: 'Agendas / registros' },
]
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <form
            onSubmit={(e) => { e.preventDefault(); setCampaign(campaignInput.trim()) }}
          ...
          <label className="flex items-center gap-1.5 text-xs text-muted-foreground cursor-pointer select-none">
            <input type="checkbox" checked={paidOnly} onChange={(e) => setPaidOnly(e.target.checked)} className="accent-brand-500" />
            Solo tráfico pago
          </label>
          <button
            onClick={exportCSV}
          ...
          <table className="w-full text-sm whitespace-nowrap">
            <thead>
              <tr className="border-b border-border text-left text-muted-foreground text-[11px] uppercase">
                ...
                {COLS.map((c) => (
                  <th key={c.label} className="px-3 py-2.5 text-right" title={c.hint}>{c.label}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.map((r) => (
                <tr key={r.date} className="border-b border-border/60 hover:bg-muted/30">
                  ...
                  {COLS.map((c) => (
                    <td key={c.label} className="px-3 py-2 text-right text-foreground">{c.fmt(r)}</td>
                  ))}
                </tr>
              ))}
            </tbody>
            {total && (
              <tfoot>
                <tr className="border-t-2 border-border text-foreground font-semibold bg-card/60">
                  ...
                  {COLS.map((c) => (
                    <td key={c.label} className="px-3 py-2.5 text-right">{c.fmt(total)}</td>
                  ))}
                </tr>
              </tfoot>
            )}
      ...
      {chartData.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className={chartBox}>
            <h3 className="text-xs font-medium text-muted-foreground mb-3">Rendimiento diario · % Registro vs % Conversión VSL</h3>
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={{ top: 5, right: 8, bottom: 0, left: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#27272a" vertical={false} />
                  <XAxis dataKey="short" tick={{ fontSize: 10, fill: '#71717a' }} axisLine={false} tickLine={false} interval="preserveStartEnd" minTickGap={16} />
                  <YAxis tick={{ fontSize: 10, fill: '#71717a' }} axisLine={false} tickLine={false} tickFormatter={(v) => `${v}%`} />
                  <Tooltip content={<ChartTooltip />} />
                  <Legend wrapperStyle={{ fontSize: 11 }} />
                  <Line dataKey="tasaRegistro" name="% Registro" stroke="#3b82f6" strokeWidth={2} dot={false} connectNulls />
                  <Line dataKey="tasaConversionVSL" name="% Conversión VSL" stroke="#ec4899" strokeWidth={2} dot={false} connectNulls />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className={chartBox}>
            <h3 className="text-xs font-medium text-muted-foreground mb-3">Rendimiento diario · Agendas (tráfico pago) vs Coste por agenda</h3>
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={chartData} margin={{ top: 5, right: 8, bottom: 0, left: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#27272a" vertical={false} />
                  <XAxis dataKey="short" tick={{ fontSize: 10, fill: '#71717a' }} axisLine={false} tickLine={false} interval="preserveStartEnd" minTickGap={16} />
                  <YAxis yAxisId="l" tick={{ fontSize: 10, fill: '#71717a' }} axisLine={false} tickLine={false} allowDecimals={false} />
                  ...
                  <Line yAxisId="r" dataKey="costeAgenda" name="Coste/Agenda" stroke="#f59e0b" strokeWidth={2} dot={false} connectNulls />
                </ComposedChart>
              </ResponsiveContainer>
            </div>
          </div>
