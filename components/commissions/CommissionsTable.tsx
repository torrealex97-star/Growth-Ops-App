  ...
  const columns = useMemo(() => [
    ...
    columnHelper.accessor('participant_type', {
      ...
      cell: ({ getValue }) => {
        const t = getValue()
        return (
          <span className={`text-xs px-2 py-0.5 rounded-full capitalize ${PARTICIPANT_COLORS[t]}`}>
            {t}
          </span>
        )
      },
    ...
    columnHelper.accessor('sales', {
      header: 'Venta',
      cell: ({ getValue }) => (
        <span className="text-muted-foreground text-sm">{(getValue() as { id?: string })?.id?.slice(0, 8) || '—'}...</span>
      ),
    }),
    ...
    columnHelper.accessor('percent', {
      header: '%',
      cell: ({ getValue }) => (
        <span className="text-muted-foreground">{formatPercent(getValue())}</span>
      ),
    }),
    columnHelper.accessor('commission_amount', {
      header: 'Importe',
      cell: ({ row, getValue }) => (
        <span className={`font-medium ${row.original.direction === 'negative' ? 'text-red-400' : 'text-emerald-400'}`}>
          {row.original.direction === 'negative' ? '-' : ''}{formatCurrency(getValue())}
        </span>
      ),
    }),
