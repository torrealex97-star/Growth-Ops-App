...
function TimingStat({ label, value, warn }: { label: string; value: string | null; warn?: boolean }) {
  ...
  return (
    <div className={`rounded-lg border px-3 py-2 ${warn ? 'border-amber-500/30 bg-amber-500/10' : 'border-border bg-muted/40'}`}>
...
export function ContractSection({ saleId }: { saleId: string }) {
  ...
  const generate = async (send: boolean) => {
    ...
    const d = await res.json().catch(() => ({}))
    setBusy(false)
    if (!res.ok) { toast.error('No se pudo generar el contrato', { description: d?.error }); return }
    if (send) {
      toast.success(d.emailed ? 'Contrato enviado por email' : 'Contrato generado', {
        description: d.emailed ? undefined : 'Copia el enlace y pásaselo al alumno (email no configurado o sin correo).',
      })
    } else {
    ...
    <div className="bg-card border border-border rounded-lg p-6 mt-4">
      ...
      {hasDistinctPayer && (
        <div className="mb-4 rounded-lg border border-amber-500/30 bg-amber-500/10 p-3">
          ...
          <div className="flex flex-wrap gap-2">
            {([['alumno', 'Solo alumno'], ['ambos', 'Alumno + pagador'], ['tomador', 'Solo pagador']] as [Recipient, string][]).map(([val, label]) => (
              <button key={val} type="button" onClick={() => setRecipient(val)}
          ...
          <p className="text-[11px] text-muted-foreground mt-2">
            En financiación debe firmar el pagador (queda obligado al pago). En full pay basta con el alumno y así evitas fricción con el pagador.
          </p>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-2 gap-x-6">
            {contract.is_reservation ? (
              <>
                <StepDot done={!!(contract.email_sent_at || contract.status !== 'pendiente')} label="Contrato de reserva enviado" at={contract.email_sent_at} />
                <StepDot done={!!contract.read_at} label="Reserva abierta por el alumno" at={contract.read_at} />
                <StepDot done={contract.status === 'firmado'} label="Contrato de reserva firmado" at={contract.signed_at} />
              </>
            ) : (
              <>
                <StepDot done={!!(contract.email_sent_at || contract.status !== 'pendiente')} label="Contrato enviado" at={contract.email_sent_at} />
                ...
                <StepDot done={!!contract.accesos_enviados_at} label="Accesos enviados" at={contract.accesos_enviados_at} />
                <StepDot done={!!contract.accesos_abiertos_at} label="Abrió la landing de accesos (click)" at={contract.accesos_abiertos_at} />
                <StepDot
          ...
          {contract.is_reservation && (
            <p className="text-xs text-amber-400">
              Reserva: se envía solo el contrato de reserva. Los accesos NO se dan aquí — se envían al completar el pago y firmar el contrato definitivo.
            </p>
          )}

          {!contract.is_reservation && contract.status === 'firmado' && contract.accesos_enviados_at == null && (
            <p className="text-xs text-amber-400">
              Firmado, pero el webhook de accesos a GoHighLevel aún no está configurado (falta la URL).
              Da los accesos manualmente hasta que se conecte.
            </p>
          )}
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input readOnly value={contract.signUrl}
            ...
            <p className="text-xs text-muted-foreground mt-1">Pégaselo al alumno por WhatsApp/Meet si no le llega el correo.</p>
          </div>
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            ...
            <a href={contract.signUrl} target="_blank" rel="noopener noreferrer"
      ...
      {payerContract && (
        <div className="mt-5 pt-5 border-t border-border space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-2 gap-x-6">
            <StepDot done={!!(payerContract.email_sent_at || payerContract.status !== 'pendiente')} label="Contrato enviado al tomador" at={payerContract.email_sent_at} />
            <StepDot done={!!payerContract.read_at} label="Abierto por el tomador" at={payerContract.read_at} />
            <StepDot done={payerContract.status === 'firmado'} label="Firmado por el tomador" at={payerContract.signed_at} />
          </div>
          <div className="flex flex-wrap gap-2 pt-1">
            {payerContract.signed_pdf_url && (
              <a href={payerContract.signed_pdf_url} target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs bg-emerald-600/20 text-emerald-300 border border-emerald-600/30 hover:bg-emerald-600/30">
                <ExternalLink className="w-3.5 h-3.5" /> Descargar PDF firmado
              </a>
            )}
            <a href={payerContract.signUrl} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs bg-muted text-foreground border border-border hover:bg-muted">
