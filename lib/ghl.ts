...
const COUNTRY_ISO: Record<string, string> = {
  ...
  'bolivia': 'BO', 'guatemala': 'GT', 'costa rica': 'CR', 'panama': 'PA', 'panamá': 'PA',
  'republica dominicana': 'DO', 'república dominicana': 'DO', 'dominican republic': 'DO',
  'honduras': 'HN', 'el salvador': 'SV', 'nicaragua': 'NI', 'puerto rico': 'PR',
  'estados unidos': 'US', 'united states': 'US', 'usa': 'US', 'eeuu': 'US',
  'portugal': 'PT', 'francia': 'FR', 'france': 'FR', 'italia': 'IT', 'italy': 'IT',
    ...
    const res = await fetch(url, {
      ...
      headers: {
        ...
        ...(process.env.GHL_ONBOARDING_WEBHOOK_SECRET
          ? { 'x-[tenant]-secret': process.env.GHL_ONBOARDING_WEBHOOK_SECRET }
          : {}),
      },
