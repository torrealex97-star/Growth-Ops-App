...
export async function extractInvoice(
  ...
  const system = `Eres un contable que extrae datos de facturas para una academia (IA WINNERS).
...
export async function contractVariablesFromText(text: string): Promise<string> {
  const system = `Eres un asistente que prepara PLANTILLAS de contrato para una empresa (IA WINNERS).
...
export async function generateScript(
  ...
  const system = `Eres el guionista de reels de @adrian.martinez.s (IA WINNERS), tono directo y con autoridad, español de España.
