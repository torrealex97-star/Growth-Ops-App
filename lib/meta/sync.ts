...
export async function runMetaSync(sb: SupabaseClient, tenantId: string): Promise<MetaSyncResult> {
  ...
  if (configs.length === 0) {
    throw new Error(
      'Faltan credenciales de Meta o el token no tiene acceso a ninguna cuenta publicitaria. Configura META_ACCESS_TOKEN (y opcionalmente META_AD_ACCOUNT_ID) en Configuración → Integraciones.'
    )
  }
