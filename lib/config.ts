...
// servidor con service-role. El env de Vercel queda como fallback para las claves
// que no se hayan configurado desde la app.
//
// AQUÍ NO SE VUELCA NADA A `process.env`. Existía un `ensureConfig(tenantId)` que lo hacía, para que
// el código que leía `process.env.X` siguiera funcionando sin cambios. Costó cara: `process.env` es
// global al proceso y nunca se limpia, así que en un cron que recorre todas las subcuentas dentro de
// la misma lambda, la segunda heredaba las credenciales de la primera y se llenaba con SUS datos,
// estampados con su propio tenant_id. Y un secreto borrado en el panel seguía vivo en memoria el
// resto de la vida de la lambda ("he borrado el App Secret y sigue dando el mismo error").
//
// La regla, ahora: la configuración se PASA como argumento (`getTenantConfig` /
// `getTenantConfigWithFallback` devuelven una instantánea aislada). Ver lib/meta/client.ts (MetaEnv),
// lib/instagram/client.ts (IgEnv), lib/youtube/client.ts (YoutubeEnv), lib/sequra/client.ts
// (SequraEnv), lib/email/resend.ts (MailEnv) y lib/calendly.ts (requireCalendlyToken).
// ─────────────────────────────────────────────────────────────────────────────
