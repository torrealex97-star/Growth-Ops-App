...
export async function buildTemplatePreviewPdf(input: {
  ...
  const meta = [
    ...
    .join('  ·  ')
  if (meta) {
    page.drawText(sanitize(meta), { x: MARGIN, y, size: 7.5, font, color: rgb(0.5, 0.5, 0.55) })
    y -= 10
  }
  ...
  // Título
  page.drawText(sanitize(input.title || 'Contrato'), {
    x: MARGIN,
    y,
    size: 15,
    font: bold,
  ...
  draw('DOCUMENTO DE MUESTRA — las condiciones y datos del alumno se completan al generar el contrato real.', {
    size: 8,
  ...
  if (input.welcome) {
    draw(input.welcome, { color: [0.25, 0.25, 0.3], gap: 10 })
  }
